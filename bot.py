...
DONE_TEXT = "Отлично! Увидимся на следующей тренировке 💪"
...
def get_main_keyboard() -> InlineKeyboardMarkup:
    """Main menu keyboard with "get workout" button."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(
                text="💪 Получить тренировку дня",
...
def back_keyboard(day: int) -> InlineKeyboardMarkup:
    """Keyboard inside the explanations view — back to workout."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(
...
async def cmd_start(message: Message) -> None:
    """Handle /start command — send welcome message with workout button."""
    await message.answer(WELCOME_TEXT, reply_markup=get_main_keyboard())


@dp.message(Command("reset"))
async def cmd_reset(message: Message) -> None:
    """Handle /reset command — reset user progress to day 1."""
    reset_user_day(message.from_user.id)
    await message.answer(
        "🔄 Прогресс сброшен! Твой следующий день — День 1 💪",
...
async def callback_main(callback: CallbackQuery) -> None:
    """Handle "get workout" button — send warmup + workout, advance day."""
    user_id = callback.from_user.id
    current_day = get_user_day(user_id)
    day_data = WORKOUTS[current_day]

    # Send warmup only for training days (not rest days)
    if not day_data["is_rest"]:
        await callback.message.answer(WARMUP)

    # Send the workout for the current day
    await callback.message.answer(
        day_data["workout"],
        reply_markup=workout_keyboard(current_day)
    )

    # Advance to the next day (wrapping at 10)
...
async def show_explanations(callback: CallbackQuery, callback_data: DayCallback) -> None:
    """Show exercise explanations for the current day (edits the message)."""
...
async def back_to_workout(callback: CallbackQuery, callback_data: DayCallback) -> None:
    """Return from explanations to the workout card (edits the message)."""
    day = callback_data.day
    day_data = WORKOUTS[day]
...
async def callback_done(callback: CallbackQuery, callback_data: DayCallback) -> None:
    """Handle "done" button — remove buttons, send confirmation."""
